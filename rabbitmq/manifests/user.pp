        ...
        present: {
            ...
            if ($password == undef) {
                if (defined(User[$name])) {
                    $user_addd = "TMPPASS=`pwgen -s 26 1`; echo \$TMPPASS > /home/${name}/.rabbitmq.password; chown ${name}:${name} /home/${name}/.rabbitmq.password; chmod 600 /home/${name}/.rabbitmq.password; echo \$TMPPASS | /usr/sbin/rabbitmqctl add_user ${name}"
                } else {
