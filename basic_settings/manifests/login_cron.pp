...
# This defined type creates `/etc/cron.allow` through concat when needed and inserts one fragment for the resource title. It is useful when cron access is intentionally limited to explicit accounts.
# lint:endignore
#
...
#
# @param user
#   User account that should be allowed to run cron jobs. The current template uses the resource title as the emitted username.
#
# @param order
