class basic_settings::package_rabbitmq(
    ...
    if ($enable) {
        ...
        exec { 'package_rabbitmq_erlang':
            command     => "printf \"deb [signed-by=/usr/share/keyrings/rabbitmq-erlang.gpg] https://dl.cloudsmith.io/public/rabbitmq/rabbitmq-erlang/deb/${os_parent} ${os_name} main\\n\" > /etc/apt/sources.list.d/rabbitmq-erlang.list; curl -s https://dl.cloudsmith.io/public/rabbitmq/rabbitmq-erlang/gpg.E495BB49CC4BBE5B.key | gpg --dearmor | sudo tee /usr/share/keyrings/rabbitmq-erlang.gpg >/dev/null",
        ...
        exec { 'package_rabbitmq_server':
            command     => "printf \"deb [signed-by=/usr/share/keyrings/rabbitmq-server.gpg] https://dl.cloudsmith.io/public/rabbitmq/rabbitmq-server/deb/${os_parent} ${os_name} main\\n\" > /etc/apt/sources.list.d/rabbitmq-server.list; curl -s https://dl.cloudsmith.io/public/rabbitmq/rabbitmq-server/gpg.9F4587F226208342.key | gpg --dearmor | sudo tee /usr/share/keyrings/rabbitmq-server.gpg >/dev/null",
