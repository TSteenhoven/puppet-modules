...
Met [`docker::nextcloud`](docker/manifests/nextcloud.pp) start je de AIO-mastercontainer achter een reverse proxy op dezelfde host. Declareer eerst `docker` en zorg voor `basic_settings::systemd`. Met `server_name` laat je Puppet de Nginx-proxy maken; declareer dan ook `nginx` en geef een geldig TLS-certificaat en de bijbehorende sleutel op. Zonder `server_name` verzorg je die HTTPS-proxy zelf. Voer daarna dezelfde applicatiedomeinnaam in de AIO-interface in en rond de domeinvalidatie en installatie af. De ingebouwde OCC-instellingen kunnen pas daarna slagen; voer Puppet opnieuw uit als de eerste run daarop vastloopt.

Voor een eenvoudige SMTP-relay is alleen `smtp_server => 'smtp.example.org'` in je bestaande `basic_settings`-declaratie voldoende. Via Hiera is dat `basic_settings::smtp_server: smtp.example.org`; de class moet ook gedeclareerd zijn. Je hoeft geen `smtp_*`-parameters op `docker::nextcloud` mee te geven, zoals het [Nextcloud-voorbeeld](examples/docker.pp) laat zien. Puppet zet via OCC uitsluitend de SMTP-modus en host. Poort, timeout, afzender, authenticatie en encryptie worden alleen beheerd als je ze opgeeft. Nextcloud gebruikt anders zijn eigen defaults of al aanwezige instellingen.

Met de optionele `smtp_*`-parameters kun je de mailconfiguratie uitbreiden, zoals bij Authentik; `smtp_host` overschrijft de centrale relay. Voor een relay met authenticatie geef je `smtp_username` en `smtp_password` op. Puppet schakelt authenticatie in zodra je een niet-lege gebruikersnaam of een niet-leeg wachtwoord opgeeft. Geef het wachtwoord als `Sensitive` door vanuit je beveiligde Hiera-data. Puppet schrijft alleen bij een verschil en slaat lege credentials over. Een eerder beheerde optionele waarde weglaten verwijdert die niet uit Nextcloud en schakelt bestaande authenticatie niet uit. Zie de [Puppet Strings](docker/manifests/nextcloud.pp) voor alle instellingen.

Nextcloud gebruikt automatisch STARTTLS als de relay dat aanbiedt. `smtp_use_tls` kan deze upgrade niet afdwingen of uitschakelen. Voor een verplicht versleutelde verbinding kies je `smtp_use_ssl => true` met de bijbehorende relaypoort, doorgaans 465. Schakel `smtp_use_ssl` en `smtp_use_tls` niet tegelijk in. Dit verschil met Authentik volgt uit de [Nextcloud-mailconfiguratie](https://docs.nextcloud.com/server/stable/admin_manual/configuration_server/email_configuration.html).

De [officiële reverse-proxyopzet](https://github.com/nextcloud/all-in-one/blob/main/reverse-proxy.md#external-using-aio-with-an-external-reverse-proxy-eg-caddy-nginx-cloudflare-proxy) gebruikt HTTP tussen Nginx en AIO's Apache-container. De admininterface gebruikt wel HTTPS met een zelfondertekend certificaat. Beide upstreams binden alleen aan `127.0.0.1`; Docker publiceert hiervoor geen externe poort. Met `admin_server_name` publiceer je de admininterface via de normale Nginx-listeners: standaard HTTPS op poort 443, met een redirect vanaf poort 80. Nginx verbindt intern met `https://127.0.0.1:<admin_port>`. Het opgegeven certificaat moet alle geconfigureerde publieke domeinnamen dekken. Zonder adminproxy gebruik je een SSH-tunnel voor beheer, bij de standaardinstellingen bijvoorbeeld `ssh -L 8080:127.0.0.1:8080 admin@cloud.example.org`, en open je `https://127.0.0.1:8080`. Gebruik bij een aangepaste `admin_port` die waarde als laatste poort in de tunnelmapping. De admininterface bestuurt via de Docker-socket containers met toegang tot de host; beperk toegang op deploymentniveau. Een read-only socketmount beperkt de Docker-API-bevoegdheden niet.
...
De gedeelde `docker::compose_exec` voert OCC als `www-data` uit in de vaste container `nextcloud-aio-nextcloud`, overeenkomstig de [AIO-documentatie](https://github.com/nextcloud/all-in-one#how-to-run-occ-commands). Deze door AIO gemaakte container heeft geen Compose-servicelabel; de selectie gebruikt daarom zijn exacte containernaam. Een ontbrekende of gestopte container of een onvoltooide installatie laat de opdracht mislukken zonder configuratie te schrijven. Een volgende Puppet-run kan het opnieuw proberen. PHP en OCC worden in de container uitgevoerd; PHP op de host is niet nodig.
